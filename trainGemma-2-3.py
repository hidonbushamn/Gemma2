...
class BorderDataset(Dataset):
    ...
    def __init__(self,data):
        self.data=data
        self.tokenizer = AutoTokenizer.from_pretrained("gemma2b")
...
class BorderDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size = 1, shuffle=True, num_workers = 8,persistent_workers=True)

    def val_dataloader(self): 
        return DataLoader(self.validation_dataset, batch_size = 1, num_workers = 8,persistent_workers=True)

class GemmaFineTuner(LightningModule):
    def __init__(self,learning_rate):
        super(GemmaFineTuner,self).__init__()
        model = AutoModelForSequenceClassification.from_pretrained( "gemma2b",torch_dtype=torch.bfloat16,num_labels=2)
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="gemma2b-{epoch:02d}-{val_loss:.2f}", #name for the checkpoint, before i was using "best-checkpoint"
    ...
    train=pd.read_csv(r'data_traing.csv',sep='|')
    val=pd.read_csv(r'data_valg.csv',sep='|')
    test_df=pd.read_csv(r'data_testg.csv',sep='|')
    data_module=BorderDataModule(train_df=train,val_df=val)
    data_module.setup()
    
    train_dls=DataLoader(BorderDataset(train),batch_size = 1, num_workers = 8,persistent_workers=True)
    val_dls=DataLoader(BorderDataset(val),batch_size = 1, num_workers = 8,persistent_workers=True)
    test_dls=DataLoader(BorderDataset(test_df),batch_size = 1, num_workers = 8,persistent_workers=True)
